...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="${ctx}/jd/list" method="post">
		<div class="searchBar">
			<table class="searchContent">
				<tr>
				    <td>记录编号：<input type="text" style="width: 150px;" name="filter_EQS_jdID" /></td>
					<td>
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
			<c:forEach items="${page.result}" var="acc">
				...
				<td><input name="djIds" value="'${acc.jdID}'" type="checkbox"></td>
				    <td>${acc.jdID}</td>
					<td>${acc.title}</td>
				...
				<td>
				<a title="删除检定记录信息" target="ajaxTodo" href="${ctx}/jd/delete/${acc.jdID}" class="btnDel">删除检定记录信息</a>
				<a title="查看检定记录信息" target="navTab" href="${ctx}/jd/viewUi/${acc.jdID}" class="btnView">查看检定记录信息</a>
				<a title="编辑检定记录信息" target="navTab" href="${ctx}/jd/editUi/${acc.jdID}" class="btnEdit">编辑检定记录信息</a>
				</td>
				</tr>
			</c:forEach>
		</tbody>
